...
import { Star, ArrowRight, TrendingUp, Zap, Award, Crown, Shield, Database, Code, Globe, Lock } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  // Filtros conforme o print solicitado
  ...
  // Prepara lista duplicada para o efeito de marquee infinito
  const marqueeChallenges = useMemo(() => {
    // Duplicamos a lista para criar o loop visual perfeito
    return [...MOCK_CHALLENGES, ...MOCK_CHALLENGES];
  }, []);

  // Auto-play para o carrossel Hero
  useEffect(() => {
      ...
      <div className="space-y-4 animate-fade-in">
        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 mb-6">
          <h3 className="text-white font-bold text-lg flex items-center gap-2">
            <Crown className="text-yellow-500" /> Melhores Forjadores
          </h3>
        ...
        <div className="space-y-3">
          {sortedUsers.map((user, index) => {
            let rankIcon;
            let rankClass = "text-slate-400 font-bold w-6 text-center";
            
            if (index === 0) {
              rankIcon = <Crown size={20} className="text-yellow-500 fill-yellow-500/20" />;
              rankClass = "";
            } else if (index === 1) {
              rankIcon = <Award size={20} className="text-slate-300 fill-slate-300/20" />;
              rankClass = "";
            } else if (index === 2) {
              rankIcon = <Award size={20} className="text-amber-700 fill-amber-700/20" />;
              rankClass = "";
            }
            ...
            return (
              <div key={user.id} className="flex items-center bg-card p-4 rounded-xl border border-slate-700/50 hover:border-slate-500 transition-colors">
                ...
                <div className="relative mr-4">
                  ...
                  {index < 3 && <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-dark"></div>}
                </div>
                <div className="flex-1">
                  <h4 className="text-white font-bold text-sm">{user.name}</h4>
                  <span className="text-xs text-slate-500">{user.role === 'Teacher' ? 'Instrutor Mestre' : `Nível ${user.level}`}</span>
                </div>
                <div className="text-right">
                  <span className="block text-primary font-black text-sm">{user.points} XP</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 rounded-3xl text-white shadow-lg relative overflow-hidden">
           <div className="relative z-10">
             <h3 className="text-xl font-black mb-2">Modo Iniciante</h3>
             <p className="text-sm opacity-90 mb-4">Comece sua jornada aqui! Desafios simplificados para aprender a lógica sem complicação.</p>
           </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {easyChallenges.length === 0 && (
             <p className="text-slate-500 text-center py-10">Nenhum desafio para iniciantes disponível no momento.</p>
          )}
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        <div className="bg-gradient-to-r from-amber-600 to-orange-600 p-6 rounded-3xl text-white shadow-lg relative overflow-hidden">
           <div className="relative z-10">
             <h3 className="text-xl font-black mb-2 flex items-center gap-2"><Crown size={24} fill="white" /> SkillForge Premium</h3>
             <p className="text-sm opacity-90 mb-4">Acesse desafios profissionais criados por empresas parceiras e ganhe certificações.</p>
             <button className="bg-white text-orange-600 px-6 py-2 rounded-full font-bold text-xs uppercase tracking-widest hover:bg-orange-50 transition-colors">
               Assinar Agora
             </button>
           </div>
        </div>
        ...
        <div>
          <h4 className="text-white font-bold mb-4">Desafios Exclusivos</h4>
          <div className="space-y-4">
             {premiumChallenges.map(c => (
               <div key={c.id} className="relative group">
                 <ChallengeCard challenge={c} />
                 {c.isLocked && (
                   <div className="absolute top-2 right-2 bg-slate-900/80 backdrop-blur text-amber-500 px-2 py-1 rounded text-[10px] font-bold flex items-center border border-amber-500/30">
      ...
      <div className="animate-fade-in">
        <div className="grid grid-cols-2 gap-4">
          {categories.map((cat) => (
            <div 
              key={cat.id}
              className={`p-6 rounded-2xl border ${cat.bg} flex flex-col items-center justify-center text-center cursor-pointer hover:scale-105 transition-transform`}
  ...
  const renderForYou = () => (
    <>
      ...
      <section className="relative overflow-hidden group mb-8 animate-fade-in">
        <div 
      ...
      {/* Categories / Trending Infinite Marquee Section */}
      <section className="space-y-4 mt-8 overflow-hidden">
        ...
        <div className="relative w-full">
            ...
            {marqueeChallenges.map((c, index) => (
                <div 
                key={`${c.id}-${index}`} 
                className="min-w-[130px] max-w-[130px] group cursor-pointer transform transition-transform hover:scale-105"
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24 md:pb-8">
      ...
      {/* Filters Bar (Style Updated to match Screenshot - Underline instead of Pills) */}
        ...
        <div className="flex gap-6 px-4 overflow-x-auto no-scrollbar">
          {filters.map((filter) => (
            <button
              ...
              className={`
                pb-3 text-sm font-bold whitespace-nowrap transition-all relative
                ${activeFilter === filter ? 'text-primary' : 'text-slate-400 hover:text-slate-200'}
              `}
            >
              {filter}
              {activeFilter === filter && (
                <span className="absolute bottom-0 left-0 w-full h-0.5 bg-primary rounded-t-full animate-slide-up"></span>
              )}
            </button>
          ))}
        </div>
